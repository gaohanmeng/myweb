...
from .models import Category, Tag, Post  # Status
...
class PostAdminForm(forms.ModelForm):
    ...
    class Meta:
        model = Post
        # fields = ('title', 'category', 'tag', 'desc', 'is_md', 'content',
        #           'content_md', 'content_ck', 'status')
        fields = ('category', 'tag', 'title', 'desc', 'is_md', 'content', 'content_md',
                  'content_ck', 'status')
    ...
    def clean(self):
        ...
        if not content:
            self.add_error(content_field_name, '必填哦')
            return
